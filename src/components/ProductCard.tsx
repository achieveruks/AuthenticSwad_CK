...
import { Star, Clock, Flame, ShoppingBag, Eye, Plus, Sparkles } from 'lucide-react';
...
export const ProductCard: React.FC<ProductCardProps> = ({ product, viewMode = 'grid' }) => {
  ...
  return (
    <motion.div
      ...
      className="group bg-white rounded-xl p-3.5 sm:p-4 border border-gray-200 hover:border-orange-500/80 shadow-xs hover:shadow-md transition-all duration-200 flex flex-col justify-between cursor-pointer"
    >
      <div>
        {/* Image Container */}
        <div className="relative aspect-4/3 w-full bg-gray-100 rounded-lg overflow-hidden mb-3">
          <img
            src={product.image}
            alt={product.name}
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-gray-900/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
          ...
          <div className="absolute top-2 left-2 right-2 flex items-center justify-between pointer-events-none">
            <div className="flex items-center gap-1.5">
              ...
              {product.inStock === false ? (
                ...
                <span className="px-2 py-0.5 bg-gray-900 text-white font-bold text-[10px] uppercase tracking-wider rounded shadow-xs">
                  CHEF SPECIAL
                </span>
              ) : null}
            </div>
            ...
            <span
              className={`inline-flex items-center justify-center w-4 h-4 bg-white/95 rounded shadow-xs border ${
          ...
          <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between text-[11px] text-white font-medium">
            <span className="px-1.5 py-0.5 bg-gray-950/70 backdrop-blur-xs rounded flex items-center gap-1 text-[10px]">
        ...
        <div>
          ...
          <div className="flex justify-between items-start mb-1 gap-2">
            ...
            <span className="text-orange-600 font-bold text-sm shrink-0">₹{product.price}</span>
          </div>
        ...
        {product.inStock === false ? (
          ...
          >
            <button
              ...
              className="w-5 h-5 rounded hover:bg-orange-200 text-orange-900 flex items-center justify-center transition-colors"
              aria-label="Decrease"
            >
              -
            </button>
          ...
          <button
            ...
            className="p-1.5 px-2.5 bg-orange-600 hover:bg-orange-700 active:bg-orange-800 text-white rounded-lg text-xs font-bold transition-colors shadow-xs flex items-center gap-1"
